...
# searches Steam grid DB or the official Steam store to get cover images for games
def getGameImage():
    global coverImage
    ...
    if gridEnabled:
        log("searching for an icon using SGDB")
            ...
            for i in range(0, len(gridIconsDict)):
                ...
                if entry[1] == False:
                    ...
                    if entry[3] == "image/png":
                        ...
                        log("successfully retrieved icon from SGDB")
            ...
            for i in range(0, len(gridIconsDict)):
                ...
                if entry[1] == False:
                    ...
                    if page.status_code != 200:
                        error(f"status code {page.status_code} received when trying to web scrape SGDB, ignoring")
                        return
                    ...
                    coverImageText = f"Art by {entry[4]} on Steam Grid DB"
    ...
    if steamStoreCoverartBackup:
        ...
        try:
            ...
            steamAppID = 0
            # loops through every game until it finds one matching your game's name
            ...
            # then load the store page, and find the icon through it
            URL = f"https://store.steampowered.com/app/{steamAppID}/"
            page = requests.get(URL, allow_redirects=True)

            # if it was redirected to the main page (Steam does this whenever it receives an invalid URL), exit
            ...
            error(f"Exception {e} raised when trying to fetch {gameName}'s icon through Steam, ignoring")
            coverImage = None
            coverImageText = None
...
def getGameDiscordID():
    ...
    # if the game was fetched using the local checker, instead of through Steam
    ...
    log(f"could not find the Discord game ID for {gameName}, defaulting to the default game ID")
    appID = defaultAppID
    return
...
def getLocalPresence():
    ...
    # loop through all games we're supposed to look for
    for game in localGames:
        # check if that game is running locally
        if game in processNames:
            # write down the process name and its creation time
        ...
        if previousGameName != gameName:
            ...
            # if the game has changed or a new game has been opened
                ...
                if isPlaying:
                    ...
                    # if the game is fetched through local tasks, the startTime has already been set
